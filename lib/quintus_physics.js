...
Quintus.Physics = function(Q) {
  ...
  Q.component('physics',{
    ...
    velocity: function(x,y) {
      if(Q._isUndefined(y)) { y = this.entity.p.vy }
      if(Q._isUndefined(x)) { x = this.entity.p.vx }
      var stage = this.entity.parent;
      this._body.SetAwake(true);
    ...
    force: function(x,y) {
      var stage = this.entity.parent;
      this._body.ApplyForce(new B2d.Vec(x / stage.world.scale,
      y / stage.world.scale),
    ...
    impulse: function(x,y) {
      var stage = this.entity.parent;
      this._body.ApplyImpulse(new B2d.Vec(x / stage.world.scale,
      y / stage.world.scale),
                    this._body.GetWorldCenter());
    },
    ...
    step: function() {
      ...
      p.x = pos.x * stage.world.scale + p.cx;
      p.y = pos.y * stage.world.scale + p.cy;
